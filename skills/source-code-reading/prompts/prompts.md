...
expected_output: 將 Markdown 閱讀筆記寫入 [清楚的 .md 輸出路徑]，至少包含範圍與入口、專案邊界（若 relevant）、實際閱讀或搜尋過的檔案、已確認、合理推論、待驗證、下一步
...
- 如果沒有指定 `entry`，skill 會先做最小盤點再選入口；但你提供越接近真實入口的線索，結果通常越準。
- 入口越具體，結果通常越好。
